...
def calculate_base_tuition(
    ...
    if non_tntt_only_count <= 0:
        discounted_total = total - float(max(viet_ngu_9_count, 0) * 35)
        return max(discounted_total, 0.0)

    normalized_diocese_id = (diocese_id or "").strip().lower()
    if "nx" in normalized_diocese_id:
        discounted_total = total + float(non_tntt_only_count * 225)
        discounted_total -= float(max(viet_ngu_9_count, 0) * 35)
        return max(discounted_total, 0.0)

    tuition_schedule = {1: 125.0, 2: 250.0, 3: 315.0}
    discounted_total = total + float(tuition_schedule.get(non_tntt_only_count, 375.0))
    discounted_total -= float(max(viet_ngu_9_count, 0) * 35)
    return max(discounted_total, 0.0)
