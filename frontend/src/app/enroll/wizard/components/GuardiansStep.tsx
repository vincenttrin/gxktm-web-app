...
export function GuardiansStep() {
  ...
  const handleDelete = (index: number) => {
    if (confirm(t('wizard.guardians.confirmRemove'))) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">{t('wizard.guardians.title')}</h2>
          <p className="mt-1 text-sm text-gray-600">
            {t('wizard.guardians.description')}
          </p>
        </div>
        {editingIndex === null && (
          <button
          ...
          >
            ...
            {t('wizard.guardians.addGuardian')}
          </button>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            ...
            {t('wizard.guardians.guardiansCount', { count: guardians.length })}
          </h3>
        </div>
        ...
        <div className="divide-y divide-gray-100">
              ...
              {editingIndex === index ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {t('wizard.guardians.relationship')}
                      </label>
                      <select
                      ...
                      >
                        <option value="">{t('wizard.guardians.selectRelationship')}</option>
                        <option value="Father">{t('wizard.guardians.relationships.father')}</option>
                        <option value="Mother">{t('wizard.guardians.relationships.mother')}</option>
                        <option value="Guardian">{t('wizard.guardians.relationships.guardian')}</option>
                        <option value="Grandfather">{t('wizard.guardians.relationships.grandfather')}</option>
                        <option value="Grandmother">{t('wizard.guardians.relationships.grandmother')}</option>
                        <option value="Other">{t('wizard.guardians.relationships.other')}</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {t('wizard.guardians.emailAddress')}
                      </label>
                      <input
                        type="email"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {t('wizard.guardians.phoneNumber')}
                      </label>
                      <input
                        type="tel"
          ...
          {editingIndex === -1 && (
            <div className="p-6 bg-gray-50">
              <h4 className="text-md font-semibold text-gray-900 mb-4">{t('wizard.guardians.addNewGuardian')}</h4>
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          {guardians.length === 0 && editingIndex !== -1 && (
            <div className="p-8 text-center">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">{t('wizard.guardians.noGuardiansYet')}</h3>
              <p className="mt-2 text-gray-500">
                {t('wizard.guardians.noGuardiansDesc')}
              </p>
            </div>
          )}
        </div>
      ...
      {guardians.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-900">{t('wizard.guardians.atLeastOneRequired')}</p>
              <p className="text-sm text-amber-700 mt-1">
                {t('wizard.guardians.atLeastOneRequiredDesc')}
              </p>
            </div>
          </div>
        </div>
      ...
      <WizardNavigation
        ...
        onNext={goToNextStep}
        backLabel={t('wizard.guardians.backToFamily')}
        nextLabel={t('wizard.guardians.continueToChildren')}
        isLoading={isLoading}
